"use client";
import { EditorBtns } from "@/types/editor_types";
import { WebPage } from "@prisma/client";
import { createContext, Dispatch, useContext, useReducer } from "react";
import { EditorAction } from "./editor_actions";
...
const editorReducer = (
  ...
  switch (action.type) {
    ...
    case "SET_WEBPAGE_ID":
      const { webPageId } = action.payload;
      const updatedEditorStateWithWebPageId = {
        ...state.editor,
        webPageId,
      };

      const updatedHistoryWithWebPageId = [
        ...state.history.history.slice(0, state.history.currentIndex + 1),
        { ...updatedEditorStateWithWebPageId }, // Save a copy of the updated state
      ];

      const webPageIdState = { ...state };
      return webPageIdState;
  }
};
  ...
  return (
    <EditorContext.Provider
      value={{
        ...
        webPageDetails: props.webPageDetails,
      }}
    >
      {props.children}
    </EditorContext.Provider>
